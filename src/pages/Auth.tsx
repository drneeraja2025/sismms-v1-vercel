// File: src/pages/Auth.tsx (FINAL FIX: Footer and Path Integration)

import { useState, useEffect } from 'react';
import { useNavigate, useLocation } from 'react-router-dom';
import { useAuth } from '../contexts/AuthContext';
import { supabase } from '../utility/SupabaseClient'; // CRITICAL PATH FIX
import Footer from '../components/Footer'; // NEW IMPORT for Footer
// Imports for UI components (CRITICAL PATH FIXES)
...
    setLoading(true);
    ...
    // CRITICAL FIX: Use synchronized Vercel domain for production stability
    const redirectToUrl = import.meta.env.PROD 
        ? 'https://epsosjswqkxjblbwwvdr.vercel.app/auth' // Vercel Domain + Auth Route
        : window.location.origin + '/auth'; // Localhost
...
  return (
    <div className="flex flex-col min-h-screen"> {/* CRITICAL: Use flex-col and min-h-screen for full height */}
        <div className="flex-grow flex items-center justify-center p-4 bg-gray-100"> {/* Main centered content area */}
...
          </Card>
        ...
        <Footer /> {/* FOOTER INTEGRATED HERE, always at the bottom */}
    </div>
  );
};
